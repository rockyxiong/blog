    ...
    <div class="result_wrap">
        <div class="result_content">
            <table class="list_tab">
                ...
                <tr>
                    <td class="tc">
                        <input type="text" onchange="changeOrder(this,{{$vo['id']}})" name="ord[]" value="{{$vo['nav_order']}}">
                    </td>
                    <td class="tc">{{$vo['id']}}</td>
                    <td>
                        <a href="#">{{$vo['nav_name']}}</a>
                    </td>
                    <td>{{$vo['nav_url']}}</td>
                    <td>
                        <a href="{{url( 'admin/navs/'.$vo->id.'/edit' )}}">修改</a>
...
<script>
    function changeOrder(obj,navs_id){
        ...
        $.post( "{{url('admin/navs/changeorder')}}",{'_token':'{{csrf_token()}}','link_order':nav_order,'id':navs_id},function(data){
            if( data.code == 0 ){
    ...
    function delNav( navs_id ){
        //询问框
        layer.confirm('你确定要删除这个分类吗？', {
            btn: ['确定','取消'] //按钮
        }, function(){
            $.post( "{{url( 'admin/navs/' )}}/"+navs_id,{'_method':'delete','_token':"{{csrf_token()}}"},function(e){
//                alert( e.msg );
